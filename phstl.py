...
ap.add_argument('-r', '--reproject', action='store', default=None, type=str, help='Reprojection EPSG')
...
if args.window != None:
    # if a geographic window is specified, add a GDAL warp to apply it
    log(f'cropping to {args.window}')
    warps.append(gdal.WarpOptions(outputBounds=(args.window[0], args.window[1], args.window[2], args.window[3])))
...
# log(f'aftr spatial ref: {img.GetSpatialRef()}')
# # print all the img attrs for reference
# print("IMG:", dir(img))
# for attr in dir(img):
#     if attr.startswith("Get"):
#         try:
#             print(attr, ":", getattr(img, attr)())
...
log(f"pixel->geo transform: {t}")

git = gdal.InvGeoTransform(t)
...
if args.pixels == None:
    ...
    log(f"bounds: xmin: {xmin}, ymin: {ymin}, xmax: {xmax}, ymax: {ymax}")
...
with stlwriter(args.STL, facetcount) as mesh:
    for y in range(mh):
        ...
        for x in range(mw):
            ...
            b = (
                ((xmin + x) * t[1]) + ((ymin + y + 1) * t[2]),
                ((xmin + x) * t[4]) + ((ymin + y + 1) * t[5]),
                (zscale * (float(bv) - zmin)) + args.base
            )

            c = (
                ((xmin + x + 1) * t[1]) + ((ymin + y) * t[2]),
                ((xmin + x + 1) * t[4]) + ((ymin + y) * t[5]),
                (zscale * (float(cv) - zmin)) + args.base
            )

            if not skip(av):
                a = (
                    ((xmin + x) * t[1]) + ((ymin + y) * t[2]),
                    ((xmin + x) * t[4]) + ((ymin + y) * t[5]),
            ...
            if not skip(dv):
                d = (
                    ((xmin + x + 1) * t[1]) + ((ymin + y + 1) * t[2]),
                    ((xmin + x + 1) * t[4]) + ((ymin + y + 1) * t[5]),
